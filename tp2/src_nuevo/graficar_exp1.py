...
esquema_big_sinPCA = "promedio_big_knn_K{0}_k{1}_size{2}_fold{3}.txt"
esquema_big_conPCA = "promedio_big_knn_K{0}_k{1}_size{2}_alpha{3}_fold{4}.txt"
...
esquema_red_sinPCA = "promedio_red_knn_K{0}_k{1}_size{2}_fold{3}.txt"
esquema_red_conPCA = "promedio_red_knn_K{0}_k{1}_size{2}_alpha{3}_fold{4}.txt"
        ...
        for K in Kfolds:
            for s in sizes:
                for j in range(1,K + 1):
                    ...
                    datos_sinPCA = leer_promedios_sinPCA(carpeta_big_sinPCA.format(knn), esquema_big_sinPCA, rango, K, s, j)
                    eje_accuracy = []
                    eje_precision = []
                    eje_recall = []
                    ...
                    datos_conPCA = leer_promedios_conPCA(carpeta_big_conPCA.format(knn,a), esquema_big_conPCA, rango, K, s, j, a)
                    eje_accuracy_conPCA = []
                    ...
                    # PCA + kNN canonico
                    datos_sinPCA = leer_promedios_sinPCA(carpeta_red_sinPCA.format(knn), esquema_red_sinPCA, rango, K, s, j)
                    eje_accuracy = []
                    eje_precision = []
                    eje_recall = []
                    ...
                    datos_conPCA = leer_promedios_conPCA(carpeta_red_conPCA.format(knn,a), esquema_red_conPCA, rango, K, s, j, a)
                    eje_accuracy_conPCA = []
                    eje_precision_conPCA = []
                    eje_recall_conPCA = []
