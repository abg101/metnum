...
path_resumen_sinPCA = "experimentos/experimento_1/promedio_mediciones_{0}/knn_{5}/promedio_{0}_{1}_K{2}_k{3}_size{6}_fold{4}.txt"
path_resumen_conPCA = "experimentos/experimento_1/promedio_mediciones_{0}/knn_{5}/promedio_{0}_{1}_K{2}_k{3}_size{6}_alpha{7}_fold{4}.txt"
sizes = [20,50,75,100]
        ...
        for a in alphas:
            for s in sizes:
                for j in range(1,k_folds + 1):
                    path = "experimentos/experimento_1/mediciones_{0}/knn_1/{1}_1/{0}_size{5}_K{2}_knn1_alpha{6}_fold{3}.csv.medidas".format(m, metodo, k_folds, j, knn,s,a)
                    if not os.path.isfile(path):
                        path = "experimentos/experimento_1/mediciones_{0}/knn_{4}/{0}_size{5}_K{2}_knn{4}_fold{3}.csv.medidas".format(m, metodo, k_folds, j, knn,s)
                        if not os.path.isfile(path):
                            break
                    ...
                    if a == 0:
                        with open(path_resumen_sinPCA.format(m,metodo,k_folds, 1,j,knn,s), mode='w') as fw:
                        ...
                        with open(path_resumen_conPCA.format(m,metodo,k_folds, 1,j,knn,s,a), mode='w') as fw:
                            fw.write("Promedio de Tiempos(en ticks):\n")  
                            fw.write("{}\n".format(Promedio_Tiempo[0]/k_folds))
                            fw.write("Promedio Accuracy general:\n")  
                            fw.write("{}\n".format(Promedio_Accuracy_general[0]/k_folds))
                    ...
                    for i in range(5, 105, 5):
                        ...
                        for j in range(1,k_folds + 1):
                            path = "experimentos/experimento_1/mediciones_{0}/knn_{4}/{1}_{7}/{0}_size{5}_K{2}_knn{4}_alpha{6}_fold{3}.csv.medidas".format(m, metodo, k_folds, j, knn,s,a,i)
                            if not os.path.isfile(path):
                                path = "experimentos/experimento_1/mediciones_{0}/knn_{4}/{0}_size{5}_K{2}_knn{4}_fold{3}.csv.medidas".format(m, metodo, k_folds, j, knn,s)
                                if not os.path.isfile(path):
                                    break
                        ...
                        if a == 0:
                            with open(path_resumen_sinPCA.format(m,metodo,k_folds, i,j,knn,s), mode='w') as fw:
                                fw.write("Promedio de Tiempos(en ticks):\n")
                                fw.write("{}\n".format(Promedio_Tiempo[0]/k_folds))
                                fw.write("Promedio Accuracy general:\n")  
                                fw.write("{}\n".format(Promedio_Accuracy_general[0]/k_folds))
                            ...
                            with open(path_resumen_conPCA.format(m,metodo,k_folds, i,j,knn,s,a), mode='w') as fw:
                                fw.write("Promedio de Tiempos(en ticks):\n")
                                fw.write("{}\n".format(Promedio_Tiempo[0]/k_folds))
                                fw.write("Promedio Accuracy general:\n")  
                                fw.write("{}\n".format(Promedio_Accuracy_general[0]/k_folds))
