...
train_size = [20,50,75,100]

path_resumen_sinPCA = "experimentos/experimento_1/promedio_mediciones_{0}/knn_{3}/K_{2}/promedio_{0}size{1}_K{2}_knn{3}.txt"
path_resumen_conPCA = "experimentos/experimento_1/promedio_mediciones_{0}/knn_{3}/K_{2}/promedio_{0}size{1}_K{2}_knn{3}_alpha{4}.txt"
#sizes = [20,50,75,100]
alphas = [0,1,5,10,15,20,25,30,35,40,45]
for m in modos:
        ...
        for k in knn:  
            for a in alphas:
                ...
                for j in range(1,k_folds + 1):
                    #print("j: "+str(j))
                    path = "experimentos/experimento_1/mediciones_{0}/knn_{3}/pca_{5}/{0}_size{4}_K{1}_knn{3}_alpha{5}_fold{2}.csv.medidas".format(m, k_folds, j, k,size,a)
                    #"experimentos/experimento_1/mediciones_{0}/knn_{3}/{0}_size{4}_K{1}_knn{3}_alpha{5}_fold{2}.csv.medidas".format(m, k_folds, j, knn,100,a)
                
                    if not os.path.isfile(path):
                        print("path")
                        path = "experimentos/experimento_1/mediciones_{0}/knn_{3}/{0}_size{4}_K{1}_knn{3}_fold{2}.csv.medidas".format(m, k_folds, j, k,size)
                        if not os.path.isfile(path):
                            print("noespath")
                            break
                ...
                if a == 0:
                    ...
                    with open(path_resumen_sinPCA.format(m,size,k_folds,k), mode='w') as fw:
                        ...
                        for i in Promedio_Accuracy_sujeto.keys():
                            fw.write("{0}: {1}\n".format(i,Promedio_Accuracy_sujeto[i]/k_folds))

                        fw.write("Promedio Precision por sujeto:\n")
                        for i in Promedio_Precision_sujeto.keys():
                            fw.write("{0}: {1}\n".format(i,Promedio_Precision_sujeto[i]/k_folds))

                        fw.write("Promedio Recall por sujeto:\n")
                        for i in Promedio_Recall_sujeto.keys():
                            fw.write("{0}: {1}\n".format(i,Promedio_Recall_sujeto[i]/k_folds))
                else:        
                    with open(path_resumen_conPCA.format(m,size,k_folds,k,a), mode='w') as fw:
                        fw.write("Promedio de Tiempos(en ticks):\n")  
                        fw.write("{}\n".format(Promedio_Tiempo[0]/k_folds))
                        fw.write("Promedio Accuracy general:\n")  
                        fw.write("{}\n".format(Promedio_Accuracy_general[0]/k_folds))
