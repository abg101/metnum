...
# pero hacemos crossvalidation sobre el conjunto de todas las imágenes
...
def armar_sujeto_x_imagen_todos(sujetos_ignorados=None):
    res = dict()
    for i in range(1, total_sujetos + 1):
        if sujetos_ignorados is None or i not in sujetos_ignorados:
...
if __name__ == "__main__":
    ...
    sujeto_x_imagen_todos = armar_sujeto_x_imagen_todos()
    ...
    indices_todos = list(sujeto_x_imagen_todos.keys())
    indices_todos.sort()
    shuffle(indices_todos)
    ...
    for i in range(0, k_folds):
        ...
        datos_fold = dict()
        datos_fold["test"] = [indices_todos[j] for j in range(0, len(indices_todos)) if i*tam_x_fold <= j < (i+1)*tam_x_fold]
        ...
        for i in datos_fold["test"]:
            if sujeto_x_imagen_todos[i] not in test_imgs_x_sujeto:
                test_imgs_x_sujeto[sujeto_x_imagen_todos[i]] = []
            test_imgs_x_sujeto[sujeto_x_imagen_todos[i]].append(i)
