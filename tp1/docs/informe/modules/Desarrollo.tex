...
\subsubsection{Sistema de ecuación a resolver} %TODO: mejor nombre
...
Como realizaremos una implementacion del modelo de navegante aleatorio y experimentaremos sobre la misma, tambien es pertinente analizar que tan bien condicionada está la matriz que usaremos para el algoritmo de PageRank. \\
...
\Procedure{Eliminación Gaussiana}{Matriz Rala $A$, vector $b$}
    \For{i = 1 hasta cantidad de filas}
    	\For{j = i+1 hasta cantidad de filas }
    		\State{$lista_{i}$ $\leftarrow$ $indice_filas$[j]}
    		\If{$lista_{col}$ == $i$ }
